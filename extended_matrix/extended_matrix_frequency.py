...
# Load extended transition matrix
P = pd.read_csv('extended_matrix_outputs/extended_transition_matrix.csv', index_col=0).values

# Define state variables
volume_states = np.arange(0, 950, 50)  # 0 to 900 in 50-step increments
due_states = np.arange(0, 4)  # 0 to max_due_days
...
for threshold in thresholds:
    ...
    for i, (v, due) in enumerate(state_space):
        ...
        # Holding cost
        holding_cost = v * holding_cost_per_cuft_per_day * due
...
# Save to CSV
df_results.to_csv("extended_matrix_outputs/threshold_policy_costs.csv", index=False)
